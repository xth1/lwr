...
void double_perms(struct genome_struct *g1, struct genome_struct *g2,
                  int offset,
                  int num_genes,
                  distmem_t *distmem)
{
...
#ifdef DEBUG
    //fprintf(outfile,"double_perms: offset: %3d  num_genes: %3d\n",
    offset,num_genes);
#endif
...
#ifdef DEBUG
    //fprintf(outfile,"classify_cycle_path:\n");
    //fprintf(outfile,"     num_adjacencies=%d, num_cycles=%d, num_gg=num_pp=%d, num_pg=%d, total=%d\n",
    n_adjacencies,
    n_cycles,
    n_pp,
    n_pg,
                      ...
                      int updatecycleinfo)
{
    ...
    Mirror     = vLeft+vRight;
#ifdef DEBUG
    //fprintf(outfile,"Chromosomes %d-%d go between vertices %d and %d\n",
...
#ifdef DEBUG
    //fprintf(outfile,"greyEdges done\n");
#endif /* DEBUG */
            ...
            if (done[i] == 0) {
                ...
                do {
                    ...
                    if (next1>=0)
                        ...
                        /* PI-CAP or GAMMA-TAIL, except there should only be
                              cycles now */
                        break;
                    }
...
#ifdef DEBUG
    //fprintf(outfile,"before reflection: chromNum:\n    ");
    for (i=0 ; i<size ; i++) {
            //fprintf(outfile,"%4d ",chromNum[i]);
        }
        //fprintf(outfile,"\n");

        //fprintf(outfile,"chromBd:  ... ");
        for (i=cnum_start ; i<=cnum_end+1 ; i++) {
            //fprintf(outfile,"%d ",chromBd[i]);
        }
        //fprintf(outfile," ...\n");
#endif
...
#ifdef DEBUG
        //fprintf(outfile,"after reflection: chromNum:\n    ");
        for (i=0 ; i<size ; i++) {
            //fprintf(outfile,"%4d ",chromNum[i]);
        }
        //fprintf(outfile,"\n");
...
#ifdef DEBUG
    //fprintf(outfile,"Arrays after flipping:\n");
    print_arrays(G);
#endif
}
...
#ifdef DEBUG
    //fprintf(outfile,"Closing path: index=%d   edge=(%d,%d)\n",
    index,i,j);
...
void addedge(graph_t *G, int i, int j, pathcounts_t *pcounts)
{
...
#ifdef DEBUG
    //fprintf(outfile,"Joining paths: indices=%d,%d   edge=(%d,%d)\n",
    index_i,index_j,i,j);
    ...
    Bcap[index_i] = new_Bcap;
    Gcap[index_i] = new_Gcap;
    Bcap[index_j] = P_NONE;
    Gcap[index_j] = P_NONE;
                        ...
                        pathcounts_t *pcounts)
{
    ...
    greyEdges[v1] = v2;
    greyEdges[v2] = v1; /* insert first edge */
    ...
    greyEdges[v3] = v4;
    greyEdges[v4] = v3; /* insert second edge */
                          ...
                          pathcounts_t *pcounts)
{
...
#ifdef DEBUG
    //fprintf(outfile,"join_pgpaths_2_cycle: i=%d j=%d\n", i, j);
    fflush(outfile);
#endif
    ...
    if (ptype[i] == ptype[j]) {
        v1i = Bcap[i];
        v2i = Gcap[i];   /* ends of 1st path */
        v1j = Bcap[j];
        v2j = Gcap[j];   /* ends of 2nd path */
    } else {
        v1i = Bcap[i];
        v2i = Gcap[i];   /* ends of 1st path */
        v1j = Gcap[j];
        v2j = Bcap[j];   /* ends of 2nd path */
    }
