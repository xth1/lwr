 ...
 * GRAPPA's invdist.c, but some of the innards have been
                     ...
                     int num_genes)
{
    ...
    //fprintf(outfile,"ERROR: calc_offset() could not locate gene %d\n", gA);

    return (-1);
}

int num_breakpoints(int *perm, int size)
{
    int i, b;
    int pA, pA1;
    int pB, pB1;
                          ...
                          graphstats_t *graphstats)
{
    ...
    b = num_breakpoints(distmem->perm, G->size);
