...
*/
void init_scenario_mem(int num_genes, int num_chromosomes, distmem_t *distmem,
                           ...
                           int **successors)
{
...
#if 0
    //fprintf(outfile,"init_scenario_bp: ");
    ...
    for (i=1; i<=num_genes_p+1; i++)
        //fprintf(outfile,"SUCC(%d)=%d\tSUCC(%d)=%d\n",
        i,ISUCCESSOR(i), -i,ISUCCESSOR(-i));
        //fprintf(outfile,"end init_scenario_bp\n");
#endif
    }
...
void print_scenario(struct genome_struct *g1, struct genome_struct *g2,
...
//    int startvalue, endvalue;
    ...
    while (d>0) {
        ...
        for (i=0 ; i<num_genes  &&  !found ; i++) {
            ...
            for (j=i ; j<num_genes && !found ; j++) {
                ...
                if ( ((d2 - d) > 1) || ((d2 - d) < -1) ) {
                    ...
                    //fprintf(outfile,"Reversal from gene %d to gene %d:\n\t", i, j );
        ...
        //fprintf(outfile, "Step %d: ", t);

        /*
        //fprintf(outfile, "Reverse from gene %d [%d] in chromosome %d to gene %d [%d] in chromosome %d: ", start, startvalue, chr1, end, endvalue, chr2);
        */
...
void clean_scenario_mem(struct genome_struct *current_genome,
                        struct genome_struct *trial_genome,
                        distmem_t *distmem)
{
    /* free memory */
...
void print_genes_nocr(int *source_perm, int num_genes)
{
    ...
    for (k = 0; k< num_genes; k++) {
        //fprintf(outfile," %4d",source_perm[k]);
    };
};
...
void free_simple_genome(struct genome_struct *genome)
{
    free(genome->genes);
}
