...
void join_pp_gg_paths(graph_t *G, pathcounts_t *pcounts)
{
    ...
    while (pcounts->num_gg > 0) {
        ...
        pp_index++;
        gg_index++;  /* skip past these now that they're joined */
    }
                       ...
                       )
{
    ...
    for (i=2 ; i<size-1 ; i+=2) {
        ...
        if ((pB != pA1) && (pA != pB1)) {
            ...
            g2a = g2genes[(i-1)/2];
            cap2a = (g2a >= lowcap1 || g2a <= -lowcap1);
            g2b = g2genes[i/2];
            cap2b = (g2b >= lowcap1 || g2b <= -lowcap1);
                     ...
                     distmem_t *distmem)
{
    ...
    distmem->components = (component_t *)
                          e_malloc((num_genes+1)*sizeof(component_t), "components");
...
*/
int mcdist_noncircular_full(struct genome_struct *g1,
                            struct genome_struct *g2,
                            int num_genes, int num_chromosomes,
                            distmem_t *distmem,
...
int mcdist_noncircular(struct genome_struct *g1,
                       struct genome_struct *g2,
                       int num_genes, int num_chromosomes,
                       distmem_t *distmem,
                       graphstats_t *graphstats)
...
int mcdist_capgraph(struct genome_struct *g1,
                    struct genome_struct *g2,
                    int num_genes, int num_chromosomes,
                    distmem_t *distmem,
                    graphstats_t *graphstats)
