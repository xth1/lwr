...
#define WATCH(x) TRACE(printf(#x "= %d\n", x);fflush(stdout))
...
void test_linkage()
{
    printf("Linkage is Ok\n");
}

void print_genome(struct genome_struct *g1, int num_genes)
{
    int i;
    PRINT("Num_genes=%d\n", num_genes);
    PRINT("Num_gene=%d\n", g1->genome_num);
...
/* Generated By Thiago, from function print_scenario_2
*/
void next_reversal(struct genome_struct *g1, struct genome_struct *g2,
                   int num_genes, int num_chromosomes,
                   int scenario_type,
                   int *revs[2], int *revs_size)
{

    distmem_t distmem;

    struct genome_struct *dest_genome = g2;
    struct genome_struct tmp_genome1, tmp_genome2;
    ...
    rearrangement_t *laststep, *thisstep;
...
/* Generated By Thiago, from function print_scenario_2
*/
void next_opt_reversal(struct genome_struct *g1, struct genome_struct *g2,
                       int num_genes, int num_chromosomes,
                       int scenario_type, int *begin, int *end)
{
    distmem_t distmem;
    ...
    /*
      WATCH(num_genes);
      WATCH(num_chromosomes);
      WATCH(scenario_type);


      print_genome(g1, num_genes);
      print_genome(g2, num_genes);
    */
    cbounds_t cb;        /* structure with chromosome boundaries */
    ...
    while (1) {
        ...
        if (thisstep->optype == S_FAIL) {
            ...
            //(outfile,"ERROR: reversals ended prematurely\n");
        } else {
            if (!(rparams->can_recap && thisstep->optype == S_FLIP)) {
                /* This is the point !!!!
                 * Get the begin and end of the reversal
                 *
                 */
                get_reversal(rparams, thisstep, begin, end);
                break;
            } else {
#if 0
                      ...
                      int scenario_type)
{
    ...
    ////fprintf(outfile,"HERE:::Step 0: (Source)\n");
    /*  print_genes(rparams->current_genome->genes,num_genes); */
    /* print_genome_multichrom_3(rparams->current_genome,
    		    rparams->num_genes,
    		    rparams->num_chromosomes,
...
/* try all reversals in order by chromosome, and then by increasing length */
int try_optrev_with_reversals(optrevparams_t *rparams,
                              int c1,  /* smallest chromosome # to try reveral in */
                              rearrangement_t *thisstep,
                              int *revs[2],
                              int *revs_size)
{
    ...
    *revs_size = 0;
    ...
    while (c1 <= max_chr) {
        ...
        WATCH(rparams->num_chromosomes);
        WATCH(rparams->num_genes);
        WATCH(maxlen0);
        WATCH(e1i);
        /* reversal length = j-i+1 = len0 + 1 */
        for (len0=0 ; len0 < maxlen0 ; len0++) {
 ...
 * from opt_printstep
 */
void get_reversal(optrevparams_t *rparams,
                  rearrangement_t *step, int *begin, int *end)
{
