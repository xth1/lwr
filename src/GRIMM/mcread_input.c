...
/* Read data file of gene orders in format described at top of this file */
...
on return:
   ...
   *genome_list      = array containing all the genomes
   ...
   nsegs             = number of bracketed segments
   seg_list          = list of those segments
                       (*seg_list)[3*i] = genome number (0,1,2,...)
                       (*seg_list)[3*i+1] = offset to start of segment
                       (*seg_list)[3*i+2] = length of segment
*/

void mcread_data(FILE *input, struct genome_struct **genome_list,
                ...
                )
{
    ...
    while (1) {
        ...
        if (NumGenomes>0) {
            if (c == ';' || c == '$') {            /* new chromosome */
                ...
                if (in_seg) {
                    /* error */
                    fprintf(stderr, "Genome %d has unterminated [\n", NumGenomes);
                    exit(-1);
                }
            ...
            if (c == '[') {
                if (in_seg) {
                    fprintf(stderr, "Genome %d has unbalanced [..[\n", NumGenomes);
                    exit(-1);
                } else {
                    in_seg++;
                    NumSegs++;
            ...
            if (c == ']') {
                if (!in_seg) {
                    fprintf(stderr, "Genome %d has unmatched ]\n", NumGenomes);
                    exit(-1);
                } else if (seg_len == 0) {
                    fprintf(stderr, "Genome %d has empty []\n", NumGenomes);
                    exit(-1);
                } else {
                    in_seg--;
                }
    ...
    *genome_list =
        (struct genome_struct *) e_malloc(NumGenomes*sizeof(struct genome_struct),
                                          "genome_list");
    ...
    *nsegs = NumSegs;
    *seg_list = SegList =
                    (int *) e_malloc(3*NumSegs*sizeof(int),
                                     "seg_list");
    ...
    while (1) {
        ...
        if (genome_num>=0) {
            ...
            if (c == ']') {
                in_seg--;
                *SegList++ = genome_num;
                *SegList++ = gnum - seg_len;   /* start of segment */
                *SegList++ = seg_len;
                continue;
            }
...
void delete_caps(struct genome_struct *genome,
...
/* adjust segment locations when delete_caps is performed in unichromosomal
 * case; all offsets need to be lowered by 1
 */
void delete_caps_segs(int nsegs, int *seg_list)
{
    int seg_no;
...
*/
int check_caps(struct genome_struct *genome,
               int num_genes, int num_chromosomes)
{
    int source;
    int *genes = genome -> genes;
...
void reformat_caps(struct genome_struct *genome,
                   int num_genes, int num_chromosomes)
{
    int source, dest;
    int *genes = genome -> genes;
...
#if 0
    //fprintf(outfile,"reformat_genes: ");
    for (source = 0; source < num_genes ; source++)
        //fprintf(outfile,"%d ",genes[source]);
        //fprintf(outfile,"\n");
        fflush(outfile);
#endif
}
