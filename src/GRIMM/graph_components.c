...
int calc_num_semiknots(graph_t *G)
{
    ...
    component_t *comp;  /* the component, if any, with the current vertex */
    ...
    for (i=size-1 ; i>=0 ; i--) {
        comp = (cc[i] < 0 ) ?
               (component_t *)NULL  :  &components[cc[i]];
